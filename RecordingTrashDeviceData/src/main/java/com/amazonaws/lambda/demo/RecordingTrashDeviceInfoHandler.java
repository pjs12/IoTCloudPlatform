...
public class RecordingTrashDeviceInfoHandler implements RequestHandler<Document, String> {
    private DynamoDB dynamoDb;
    private String DYNAMODB_TABLE_NAME = "TrashDeviceData";
    ...
    private String persistData(Document document) throws ConditionalCheckFailedException {
        ...
        // SpareSpace와 LockState 값이 이전상태와 동일한 경우 테이블에 저장하지 않고 종료 
        if (document.current.state.reported.SpareSpace.equals(document.previous.state.reported.SpareSpace) && 
                document.current.state.reported.LockState.equals(document.previous.state.reported.LockState)) {
                return null;
        }
        ...
        return this.dynamoDb.getTable(DYNAMODB_TABLE_NAME)
                .putItem(new PutItemSpec().withItem(new Item().withPrimaryKey("deviceId", document.device)
                        ...
                        .withString("SpareSpace", document.current.state.reported.SpareSpace)
                        .withString("LockState", document.current.state.reported.LockState)
